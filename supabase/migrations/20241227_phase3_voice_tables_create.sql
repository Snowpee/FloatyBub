-- Phase 3: Voice Tables Creation
-- Create voice_models and voice_settings tables
...
-- Add comments for voice_settings table
COMMENT ON TABLE voice_settings IS '用户语音设置表';
COMMENT ON COLUMN voice_settings.user_id IS '用户ID';
...
COMMENT ON COLUMN voice_settings.metadata IS '额外设置数据';
...
-- Create RLS policies for voice_settings (users can only access their own settings)
...
BEGIN
    ...
    IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'voice_models') THEN
        RAISE EXCEPTION 'Phase 3 migration failed: voice_models table was not created';
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'voice_settings') THEN
        RAISE EXCEPTION 'Phase 3 migration failed: voice_settings table was not created';
    END IF;
    
    RAISE NOTICE 'Phase 3 migration completed successfully: voice_models and voice_settings tables created';
END $$;