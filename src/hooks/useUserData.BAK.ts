...
// 全局上传同步单飞锁，跨多个 useUserData 实例共享
...
export const useUserData = () => {
      ...
      try {
        ...
        if (data) {
          ...
          const updatedSessions = currentSessions.map(currentSession => {
            ...
            if (polledSession) {
              const localTime = currentSession.updatedAt ? new Date(currentSession.updatedAt as any).getTime() : 0
              const cloudTime = polledSession.updatedAt ? new Date(polledSession.updatedAt as any).getTime() : 0
              const shouldUpdateFromCloud = cloudTime > localTime
              return {
                ...currentSession,
                title: shouldUpdateFromCloud ? polledSession.title : currentSession.title,
                isHidden: shouldUpdateFromCloud ? polledSession.isHidden : currentSession.isHidden,
                isPinned: shouldUpdateFromCloud ? polledSession.isPinned : currentSession.isPinned,
                updatedAt: shouldUpdateFromCloud ? polledSession.updatedAt : currentSession.updatedAt
              }
            }
          ...
          const deletedMap = new Map<string, any>((deletedRows || []).map((r: any) => [r.id, r.deleted_at]))
          const now = Date.now()
          const GRACE_PERIOD_MS = 60000

          const filteredSessions = updatedSessions.filter(local => {
            if (!deletedMap.has(local.id)) return true
            ...
            const awaitingAI = lastMsg ? (lastMsg.role === 'user' && !hasStreaming && (now - lastMsgTs) <= GRACE_PERIOD_MS) : false
            const shouldRemove = !locked && !hasStreaming && !hasPendingUpload && !awaitingAI && age >= GRACE_PERIOD_MS
  ...
  const syncToCloud = useCallback(async (retryCount = 0) => {
    ...
    if (syncToCloudLockRef.current || GLOBAL_SYNC_TO_CLOUD_LOCK) {
      console.warn('SYNC_TO_CLOUD_SKIPPED_LOCK', { userId: user.id, retryCount, at: new Date().toISOString() })
      return
    }
    ...
    console.warn('SYNC_TO_CLOUD_START', { syncId, userId: user.id, retryCount, sessionCount: chatSessions.length, at: new Date().toISOString() })
    ...
    try {
      ...
      try {
        ...
        const syncedSessionIds = new Set(updatedSessions.map(s => s.id))
        const finalizedSessions = clearedSessions.map(s => (
          syncedSessionIds.has(s.id) ? { ...s, pendingUpload: false, lastSyncedAt: new Date() } : s
        ))
      ...
      console.warn('SYNC_TO_CLOUD_DONE', { syncId, durationMs: syncDuration, sessionsCount: sessionsData.length, messagesCount: allMessages.length, at: new Date().toISOString() })
  ...
  const verifyDatabaseSync = useCallback(async (localMessages: ChatMessage[]) => {
    ...
    if (inconsistentCount === 0) {
      ...
      // 处理云端缺失（软删除或尚未上传）的本地消息：仅在满足严格条件时才移除
      const dbIdSet = new Set(dbMessages.map(m => m.id))
      
      // 建立消息ID到会话ID的映射，便于判断锁定与流式状态
      const currentSessionsState = useAppStore.getState().chatSessions
      const messageSessionMap: Record<string, string> = Object.create(null)
      const messageMetaMap: Record<string, { pendingUpload?: boolean; isStreaming?: boolean; ts: number }> = Object.create(null)
      for (const s of currentSessionsState) {
        for (const m of s.messages || []) {
          messageSessionMap[m.id] = s.id
          const ts = (() => {
            try {
              return m.message_timestamp ? new Date(m.message_timestamp).getTime() : (m.timestamp ? new Date(m.timestamp as any).getTime() : 0)
            } catch { return 0 }
          })()
          messageMetaMap[m.id] = { pendingUpload: (m as any).pendingUpload, isStreaming: !!m.isStreaming, ts }
        }
      }
      
      const now = Date.now()
      const GRACE_PERIOD_MS = 60000
      const protectedMissingIds: string[] = []
      const removableMissingIds: string[] = []
      
      for (const localMsg of localMessages) {
        if (dbIdSet.has(localMsg.id)) continue
        const meta = messageMetaMap[localMsg.id] || { pendingUpload: false, isStreaming: false, ts: 0 }
        const sessionId = messageSessionMap[localMsg.id]
        const isLocked = sessionId ? getLockedSessions().includes(sessionId) : false
        const isAwaitingAI = localMsg.role === 'user' && !meta.isStreaming && (now - meta.ts) <= 60000
        const isRecent = (now - meta.ts) < GRACE_PERIOD_MS
        const shouldProtect = isLocked || meta.isStreaming || meta.pendingUpload === true || isAwaitingAI || isRecent
        if (shouldProtect) {
          protectedMissingIds.push(localMsg.id)
        } else {
          removableMissingIds.push(localMsg.id)
        }
      }
      
      if (protectedMissingIds.length > 0) {
        console.log('⏳ [数据库验证] 保留云端缺失的本地消息（处于保护窗口或锁定/流式/待上传/等待AI）:', protectedMissingIds)
      }
      
      if (removableMissingIds.length > 0) {
        console.warn(`🗑️ [数据库验证] 检测到 ${removableMissingIds.length} 条消息在云端缺失，保持本地数据不移除`)
      }
  ...
  const syncFromCloud = useCallback(async (attempt = 1) => {
    ...
    const syncId = `${user.id}-${Date.now()}`
    const syncStartTime = Date.now()
    console.warn('SYNC_FROM_CLOUD_START', { syncId, userId: user.id, attempt, at: new Date().toISOString() })
    ...
    try {
      ...
      cloudSessions.forEach(cloudSession => {
        ...
        if (!localSession) {
          ...
          if (localMessageCount > cloudMessageCount) {
            const UNSYNCED_LOCAL_THRESHOLD_MS = 60000
            ...
            mergedSessions.set(cloudSession.id, {
              ...
              title: cloudTime > localTime ? cloudSession.title : localSession.title,
              isHidden: cloudTime > localTime ? cloudSession.isHidden : localSession.isHidden,
              isPinned: cloudTime > localTime ? cloudSession.isPinned : localSession.isPinned,
      ...
      // 根据 deleted_at 标记移除本地会话
      const { data: deletedSessionsRows } = await supabase
        .from('chat_sessions')
        .select('id, deleted_at')
        .eq('user_id', user.id)
        .not('deleted_at', 'is', null)

      const deletedSessionMap = new Map<string, any>((deletedSessionsRows || []).map((r: any) => [r.id, r.deleted_at]))
      const now = Date.now()
      const REMOVAL_GRACE_PERIOD = 60000
      for (const [id, localSession] of Array.from(mergedSessions.entries())) {
        if (!deletedSessionMap.has(id)) continue
        ...
        const lastMsg = messages[messages.length - 1]
        const lastMsgTs = lastMsg ? (lastMsg.message_timestamp ? new Date(lastMsg.message_timestamp).getTime() : (lastMsg.timestamp ? new Date(lastMsg.timestamp as any).getTime() : 0)) : 0
        const awaitingAI = lastMsg ? (lastMsg.role === 'user' && !hasStreaming && (now - lastMsgTs) <= REMOVAL_GRACE_PERIOD) : false
      ...
      setLastSyncTime(new Date())
      setSyncing(false)
      console.warn('SYNC_FROM_CLOUD_DONE', { syncId, sessionsCount: finalSessions.length, messagesCount: finalTotalMessages, at: new Date().toISOString() })
  ...
  useEffect(() => {
    ...
    const { tempSessionId, currentSessionId } = useAppStore.getState()
    const isActiveTemp = !!tempSessionId && tempSessionId === currentSessionId
  ...
  useEffect(() => {
    ...
    const nowTs = Date.now()
    const newSessionIds = chatSessions
      .filter(s => (s.messages?.length || 0) === 0 && s.pendingUpload === true && s.createdAt && (nowTs - new Date(s.createdAt as any).getTime()) <= recentMs)
      .map(s => s.id)
    if (newSessionIds.length > 0) {
      if (debouncedSyncToCloud.current) {
        clearTimeout(debouncedSyncToCloud.current)
      }
      const sinceLast = nowTs - (lastSyncToCloudTime.current || 0)
      if (sinceLast >= 1000 && !isActiveTemp) {
        lastSyncToCloudTime.current = nowTs
        syncToCloud()
      }
      return
    }
    ...
    const effectiveNewUserMessageSessionIds = tempSessionId ? newUserMessageSessionIds.filter(id => id !== tempSessionId) : newUserMessageSessionIds
    
    if (effectiveNewUserMessageSessionIds.length > 0) {
      // 用户消息创建时立即同步，不受流式检测和防抖限制
      console.log('🚀 用户消息立即同步触发', { sessions: effectiveNewUserMessageSessionIds })
      // 为“等待AI响应”场景添加短期会话锁，避免误删
      effectiveNewUserMessageSessionIds.forEach(id => lockSession(id, '等待AI响应'))
      if (debouncedSyncToCloud.current) {
        clearTimeout(debouncedSyncToCloud.current)
      }
      const nowTs = Date.now()
      ...
      if (sinceLast >= 1000) {
        ...
        console.warn('SKIP_SYNC_USER_MESSAGE_RATE_LIMIT', { sinceLast })
      }
      return
    }
    ...
    if (isMessageCompletion) {
      ...
      if (sinceLast >= 1000 && !isActiveTemp) {
        ...
        console.warn('SKIP_SYNC_COMPLETION_RATE_LIMIT', { sinceLast, isActiveTemp })
      }
    } else {
